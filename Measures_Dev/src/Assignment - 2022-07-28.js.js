...
function days_diff_in_month_Diff(d1, d2) {
    var months;
...
function eligible_visits(patient) {
...
function check_visit_has_code(visits,codes){
   console.log(visits);
   console.log(codes);
   var result = _.some(visits, function(v) { return codes.includes(v["code"]) });
console.log(result);
}
...
//console.log( days_diff_in_month_Diff(new Date(reporting_startdate),new Date(reporting_enddate)));
...
var visits=eligible_visits(patient);
var codes=oid_get_codes(OidDictionary,"2.16.840.1.113883.3.526.3.402","CPT")
check_visit_has_code(visits,codes)
